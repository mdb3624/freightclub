...
- ~~Pre-commit hook scope~~ **Fixed locally 2026-07-20:** `.git/hooks/pre-commit` now blocks commits directly to `main` (matching the documented script in `docs/OPERATIONS.md`) before running the existing `Story_Map.md` validation. **Caveat:** `.git/hooks/` is not version-controlled — this fix applies only to the clone it was made in. Every other clone/machine still needs the hook installed via the one-time setup script in `docs/OPERATIONS.md` until/unless it's wired into a tracked installer (e.g. a `postinstall`/setup script other devs actually run) — GitHub branch protection (Layer 1) remains the only enforcement that's guaranteed to apply everywhere.
...
- ~~Dangling file references~~ **`docs/roles/SHIPPER_HFD_RULES.md` created 2026-07-20** — distilled from already-shipped US-820–US-825 work and the locked Style Guide §6/§7, so this was documentation of real decisions, not new design.
- **`docs/standards/ADMIN_DESIGN_SYSTEM.md` and `docs/roles/ADMIN_HFD_RULES.md` still don't exist — deliberately.** Admin persona has zero real precedent in this repo: no components, no tokens, no design specs; both Admin stories (US-750, US-751) are `MIGRATION_PENDING`, never built. Every "Admin" mention elsewhere in these docs is templated placeholder text with no actual values behind it. Creating these files now would mean inventing a design system nobody has approved, which the HFD Gate Check exists to prevent ("PROHIBITED from finalizing a UI design until BA has provided Business Rules"). Create them for real, with actual BA/HFD design work, when an Admin story actually kicks off — not preemptively.
