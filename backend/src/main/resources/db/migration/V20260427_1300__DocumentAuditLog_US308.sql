...
CREATE TABLE document_audit_log (
  ...
  CONSTRAINT fk_audit_document FOREIGN KEY (document_id) REFERENCES load_documents(id),
...
CREATE POLICY document_audit_log_select_isolation ON document_audit_log
...
CREATE INDEX idx_audit_document_tenant_created
  ON document_audit_log(document_id, tenant_id, created_at DESC);
CREATE INDEX idx_audit_tenant_created
  ON document_audit_log(tenant_id, created_at DESC);
CREATE INDEX idx_audit_user_created
  ON document_audit_log(user_id, created_at DESC);
