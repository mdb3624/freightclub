...
class LoadAnalyticsServiceTest {
  ...
  @Test
  void testRecordLoadAnalytics_StoresTenantId() {
    OffsetDateTime postedAt = OffsetDateTime.now(ZoneOffset.UTC);

    service.recordLoadAnalytics(
        TEST_LOAD_ID, TEST_SHIPPER_ID, postedAt, 10, 150);

    verify(repository, times(1)).save(argThat(analytics ->
        TEST_TENANT_ID.equals(analytics.getTenantId())
            && TEST_LOAD_ID.equals(analytics.getLoadId())
            && TEST_SHIPPER_ID.equals(analytics.getShipperId())));
  }

  @Test
  void testGetAdminAnalytics_With30DayRange() {
    when(repository.countPostedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(500L);
    when(repository.countClaimedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(250L);
    when(repository.avgClaimTimeSecondsSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class)))
        .thenReturn(14400.0); // 4 hours

    LoadAnalyticsService.AdminAnalyticsResponse response =
        service.getAdminAnalytics(TEST_TENANT_ID, 30);

    assertNotNull(response);
    assertEquals(500, response.totalPosted());
    assertEquals(250, response.totalClaimed());
    assertEquals(50, response.claimPercentage());
    assertEquals(4.0, response.avgClaimTimeHours());
  }

  @Test
  void testGetAdminAnalytics_With90DayRange() {
    when(repository.countPostedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(1000L);
    when(repository.countClaimedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(750L);
    when(repository.avgClaimTimeSecondsSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class)))
        .thenReturn(3600.0); // 1 hour

    LoadAnalyticsService.AdminAnalyticsResponse response =
        service.getAdminAnalytics(TEST_TENANT_ID, 90);

    assertNotNull(response);
    assertEquals(1000, response.totalPosted());
    assertEquals(750, response.totalClaimed());
    assertEquals(75, response.claimPercentage());
    assertEquals(1.0, response.avgClaimTimeHours());
  }

  @Test
  void testGetAdminAnalytics_HighClaimPercentage() {
    when(repository.countPostedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(100L);
    when(repository.countClaimedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(95L);
    when(repository.avgClaimTimeSecondsSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class)))
        .thenReturn(1800.0); // 30 minutes

    LoadAnalyticsService.AdminAnalyticsResponse response =
        service.getAdminAnalytics(TEST_TENANT_ID, 7);

    assertNotNull(response);
    assertEquals(95, response.claimPercentage());
  }

  @Test
  void testGetAdminAnalytics_NullAvgClaimTime() {
    when(repository.countPostedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(100L);
    when(repository.countClaimedSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class))).thenReturn(50L);
    when(repository.avgClaimTimeSecondsSince(eq(TEST_TENANT_ID), any(OffsetDateTime.class)))
        .thenReturn(0.0);  // Service expects Double, return 0.0 for null case

    LoadAnalyticsService.AdminAnalyticsResponse response =
        service.getAdminAnalytics(TEST_TENANT_ID, 7);

    assertNotNull(response);
    assertEquals(0.0, response.avgClaimTimeHours());
  }

  @Test
  void testGetShipperAnalytics_EmptyResponse() {
    when(repository.findByShipperInDateRange(
            eq(TEST_TENANT_ID), eq(TEST_SHIPPER_ID), any()))
  ...
  @Test
  void testRecordLoadClaim_UpdatesAnalyticsRecord() {
    String truckerId = "trucker-222";
    OffsetDateTime postedAt = OffsetDateTime.now(ZoneOffset.UTC).minusHours(1);
    OffsetDateTime claimedAt = OffsetDateTime.now(ZoneOffset.UTC);
    ...
    when(repository.findById(TEST_LOAD_ID)).thenReturn(Optional.of(analytics));
    when(repository.save(any(LoadAnalytics.class)))
