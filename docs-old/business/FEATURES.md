...
Commission is calculated and deducted at **Load Settlement** (DELIVERED → SETTLED transition), not at claim.
...
SETTLED State (Automatic transition or Admin approval):
  ...
  └─ Three immutable transactions written (atomic):
     ├─ LOAD_SETTLEMENT: $1,000.00 (gross)
     ├─ PLATFORM_COMMISSION: $20.00 (2%)
     └─ TRUCKER_PAYOUT: $980.00 (net)
  └─ Payment intent created
  └─ ACH payout scheduled (2-3 business days)
```
