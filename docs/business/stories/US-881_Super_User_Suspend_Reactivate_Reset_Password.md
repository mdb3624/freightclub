...
- BR-4: Force-password-reset invalidates the user's current password and current sessions, and triggers the platform's existing password-reset-email flow (if email is configured — see known gap: production has no email provider configured today, tracked separately, not this story's problem to fix) so the user can set a new one; it never lets the Super User set or view the user's new password directly.
