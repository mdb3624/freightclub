...
| US-854 | Per-Load Diesel Fuel Cost Resolution | READY_FOR_DESIGN | Cross | — | ✅ BA Gate 1 approved by User 2026-07-13. ✅ ARCH design complete (`docs/architecture/US-854_Diesel_Cost_Resolution_Design.md`) — no migration needed (no new entity/table, `Load.originState` already exists); new `StateToEiaRegionResolver` (5-region mapping, distinct from `Load.stateToRegion()`'s unrelated 7-category lane-matching scheme) + `resolveDieselPriceForLoad`/`calculateMinimumRPM(truckerId, originState)` overloads on `CarrierCostProfileService`. Key finding: `calculateMinimumRPM` moves from a once-per-page-request threshold to a per-load calculation inside `LoadService.listOpenLoads`'s filter step. Preceded by a 5-persona "council" review + empirical validation: real EIA data (60 weeks) shows regional diesel spread averages 38¢/gal (EAST vs SOUTH) and 90¢+/gal across all 5 regions every single week. Next: HFD to design display of region/as-of-date/fallback indicator on the load board. | Jira: FREIG-116 |
