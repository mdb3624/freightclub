...
| US-854 | Per-Load Diesel Fuel Cost Resolution | BACKLOG | Cross | — | 📝 BA draft complete (`docs/business/stories/US-854_Per_Load_Diesel_Cost_Resolution.md`) — NOT yet approved, do not proceed to ARCH. Preceded by a 5-persona "council" review + empirical validation: real EIA data (60 weeks) shows regional diesel spread averages 38¢/gal (EAST vs SOUTH) and 90¢+/gal across all 5 regions every single week — a static carrier home-region misprices long-haul loads by $60+ (e.g. NYC→Tampa). v1 scope: origin-region-only dynamic lookup + price transparency (region + as-of date shown); multi-region mileage blending explicitly deferred to a follow-up story. Awaiting user approval of ACs (Gate 1) before status moves to READY_FOR_DESIGN. | Jira: FREIG-116 |
