...
| US-866 | Breached-Password Screening on Registration | READY_FOR_DESIGN | Cross | — | Surfaced via `/council-review` (2026-08-27) on a proposed password-strength meter + composition rules. Council converged (citing NIST SP 800-63B-4) that composition rules are a prohibited anti-pattern and breach-corpus screening is the actual SHALL-level control; minimum length increase was separately evaluated and rejected on carrier mobile-typing-burden grounds. Frontend strength-meter UI and any length/composition change are explicitly out of scope for this story. Gate 1 approved by Mike 2026-08-27. ⚠️ **CHG-866:** narrowed from registration+password-change to registration-only — no password-change flow exists anywhere in the backend (`passwordEncoder.encode()` called exactly once, at registration). ⚠️ Jira mirroring deferred — FREIG Atlassian instance returned "deactivated due to inactivity" (503); backfill required before DONE. Story doc: `docs/business/stories/US-866_Breached_Password_Screening.md`. | Jira: PENDING (instance deactivated) |
