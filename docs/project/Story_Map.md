...
| US-854 | Per-Load Diesel Fuel Cost Resolution | IN_PROGRESS | Cross | — | ✅ BA Gate 1 approved 2026-07-13. ✅ ARCH design complete (`docs/architecture/US-854_Diesel_Cost_Resolution_Design.md`) — no migration needed, new `StateToEiaRegionResolver` + `resolveDieselPriceForLoad`/`calculateMinimumRPM(truckerId, originState)` overloads on `CarrierCostProfileService`; `calculateMinimumRPM` moves from once-per-page to per-load inside `LoadService.listOpenLoads`. ✅ HFD design complete (`docs/hfd/US-854_Design_Spec.md`), READY_FOR_CODER — non-interactive text caption on existing `LoadBoardTable.tsx` metadata row, zero new colors/tokens, no new touch target. Real-device (CARRIER_HFD_RULES.md Phase 4) verification explicitly WAIVED by user 2026-07-14 (justification: no interactive element introduced, waiver scoped to this story only). Preceded by empirical validation: real EIA data (60 weeks) shows regional diesel spread averages 38¢/gal (EAST vs SOUTH) and 90¢+/gal across all 5 regions every single week. Next: CODER implementation. | Jira: FREIG-116 |
