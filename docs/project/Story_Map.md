...
| US-854 | Per-Load Diesel Fuel Cost Resolution | READY_FOR_REVIEWER_RE_AUDIT | Cross | — | ✅ BA Gate 1 approved 2026-07-13. ✅ ARCH design (`docs/architecture/US-854_Diesel_Cost_Resolution_Design.md`) — no migration, new `StateToEiaRegionResolver` (real EIA PADD state map) + `DieselPriceResolution` + `resolveDieselPriceForLoad`/`calculateMinimumRPM(truckerId, originState)` overloads. ✅ HFD design (`docs/hfd/US-854_Design_Spec.md`) — non-interactive caption on `LoadBoardTable.tsx`, Phase 4 device test explicitly WAIVED by user 2026-07-14. ✅ CODER complete: `LoadService.listOpenLoads` refactored from a once-per-page threshold to per-load resolution (existing `MinRpmFilteringTest` updated to match); `LoadSummaryResponse` gains `regionUsed`/`asOfPeriod`/`isFallback`; frontend caption implemented with all 5 EIA region labels + fallback + omitted states. Coverage: StateToEiaRegionResolver 100%, CarrierCostProfileService 82.1%, LoadSummaryResponse 80%, LoadService 80.6% branch (all ≥80% gate). ✅ Config-wiring bug found + fixed in REVIEWER pass 2026-07-14: `docker-compose.test.yml`/`application.yml` never bound `EIA_API_KEY`/`EIA_ENABLED` at all, so mocked tests passed while the live feature returned `available:false` in every environment — now wired and verified against real EIA data. ✅ Playwright golden-path spec added (`frontend/e2e/design-system/US-854-diesel-region-caption.spec.ts`, 2/2 passing) covering AC-1/AC-2 (region override + as-of date) and AC-3 (fallback indicator), seeding real data through the actual backend (not mocked) — resolves the earlier disclosed gap; the prior "datetime-local blocks Playwright" framing was incorrect, it only blocked the browser-use MCP manual-exploration tool, not real Playwright `fill()`. Evidence: `test-results/evidence/US-854-diesel-region-caption.png`, `US-854-fallback-indicator.png`. Tests: 902/902 backend, 291/291 + 4 new frontend unit, 106/106 E2E (104 existing + 2 new, 0 regressions), clean `tsc --noEmit`, all GH Actions CI checks green on PR #37. | Jira: FREIG-116 |
