...
2. **Capability Duplicate Check (mandatory, added 2026-08-27):** Before drafting Acceptance Criteria, grep `docs/project/Story_Map.md` for the story's core capability/feature keyword — check every status, not just DONE (a story stuck in `PARTIAL`/`READY_FOR_REVIEWER_RE_AUDIT`/`MIGRATION_PENDING` is still committed, routed work, not a clean slate). If a plausible match exists, don't draft a second story for the same user-facing capability — either extend the existing story's scope or escalate to LIBRARIAN if the existing story is stalled/abandoned and needs to be revived instead of duplicated. **Root incident this exists to catch:** US-761 (Phase 7) had already implemented the shipper dashboard's activeShipments/onTimeCarrierPct/estimatedCostPerMile KPI aggregate, stuck at `READY FOR REVIEWER RE-AUDIT` — BA drafted US-820 (Phase 10) for the same capability without checking, and it was rebuilt from scratch with an independently-invented "active load" definition that silently diverged from the original, producing a real production bug months later (see `project_us761_us820_duplicate_kpi_debt.md`). ARCHITECT's own Platform Reuse Check (step 5) and CODER's Service Reuse Check (step 5) both run this same grep later in the pipeline — catching it here, before a story is even drafted, is strictly cheaper than catching it at design or implementation time.
3. **Define Value:** Use the format: "As a [Role], I want to [Action], so that [Value]."
4. **Consult the Domain Knowledge Base:** Before writing Acceptance Criteria, check `docs/business/domain-knowledge/TRUCKING_BEST_PRACTICES.md` for entries relevant to this story's topic (carrier vetting, load matching, rate negotiation, compliance, etc.). If a relevant entry exists, cite it by name directly in the story (e.g., "Per KB: carrier-trust-signals") — don't restate the fact without the pointer. If the story's topic isn't covered and you believe it should be, don't do ad-hoc live research to fill the gap — log it in the Decision Log (Tier B, step 6 below) and let a deliberate research pass add the entry later. This is a lightweight, curated-KB-only check — not a live web search step; see `docs/changes/CHG-863.md` for why the scope stops there for now.
5. **Define Success:** Write Gherkin-style (Given/When/Then) acceptance criteria. **Any AC that encodes an external "industry best practice" claim must cite its source** — a KB entry name or a real, dated URL — directly in the story doc. An AC that says "per industry best practice" with no source attached is not acceptable; REVIEWER cannot audit an unsourced claim.
6. **INVEST Self-Check:** Before Gate 1, verify the story against all six criteria — check the box only if all pass, otherwise revise the story first:
   ...
   - [ ] **Testable** — the Gherkin ACs from step 5 are concrete enough to pass/fail unambiguously
...
- **Story_Map row at ID assignment, not at merge (MANDATORY — effective 2026-07-27):** The moment you pick a new story ID — even for a DRAFT/BACKLOG story on a branch you don't intend to finish immediately — add its row to `Story_Map.md` in the same commit as the story doc, even if every other field is still a placeholder. **Why this is non-negotiable:** an ID that exists only in a story doc on an unmerged branch is invisible to everyone else — nothing stops a later BA invocation from independently choosing the same ID for unrelated work. This happened three times from a single uncataloged branch (`feature/US-849-carrier-network-epic`, 2026-07-04) before it was discovered and recovered as CHG-864 on 2026-07-25 — three IDs had already been reused and shipped under different stories by the time anyone noticed. Story_Map.md's own duplicate-ID validator can't catch this class of collision because it only ever sees one branch's rows at a time.
