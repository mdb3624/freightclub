...
- Name: shipper-preferred-carriers.spec.ts >> Shipper Preferred Carrier List - US-707 >> US-707 AC-1: Shipper can add carrier to preferred list
- Location: e2e\shipper-preferred-carriers.spec.ts:25:3
...
Locator: locator('[data-testid="preferred-carriers-page"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="preferred-carriers-page"]')
...
```ts
  ...
  3   | 
  4   | /**
  5   |  * US-707: Shipper Preferred Carriers
  6   |  *
  ...
  13  |  */
  14  | test.describe('Shipper Preferred Carrier List - US-707', () => {
  ...
  23  |   });
  24  | 
  25  |   test('US-707 AC-1: Shipper can add carrier to preferred list', async ({ page, request }) => {
  ...
  34  | 
  35  |     try {
  36  |       // Navigate to preferred carriers page
  37  |       await page.goto('/settings/preferred-carriers', { waitUntil: 'networkidle' });
  38  | 
  39  |       // Verify page loads
> 40  |       await expect(page.locator('[data-testid="preferred-carriers-page"]')).toBeVisible({ timeout: 5000 });
      |                                                                             ^ Error: expect(locator).toBeVisible() failed
  41  | 
  42  |       // Click "Add Carrier" button
  43  |       const addButton = page.locator('[data-testid="add-carrier-btn"]');
  44  |       await expect(addButton).toBeVisible();
  45  |       await expect(addButton).toBeEnabled();
