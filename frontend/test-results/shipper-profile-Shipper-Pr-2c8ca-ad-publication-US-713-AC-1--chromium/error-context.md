...
- Name: shipper-profile.spec.ts >> Shipper Profile Setup - Golden Path (US-713) >> should complete shipper profile and allow load publication (US-713 AC-1)
- Location: e2e\shipper-profile.spec.ts:34:3
...
Locator: locator('[data-testid="profile-incomplete-alert"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="profile-incomplete-alert"]')
...
```ts
  1   | /**
  2   |  * Shipper Profile Setup Tests (US-713)
  3   |  *
  4   |  * Refactored Features (Phase 5 Pattern Rollout):
  5   |  * 1. Uses data-testid selectors (mandatory per testing_standards.md)
  6   |  * 2. Web-first assertions instead of hard-coded waits
  7   |  * 3. API-driven test data setup (TestDataSeeder) instead of UI-driven login
